...
pub fn run_claw() {
    println!("\nHermes OpenClaw Migrator");
    println!("══════════════════════════\n");
    println!("Providing seamless workspace migrations from legacy OpenClaw to Hermes Agent.");
    println!();
    ...
    if let Ok(legacy_yaml) = fs::read_to_string(&claw_path) {
        ...
        match save_config(&current_config) {
            Ok(()) => {
                println!("✓ Legacy profile migrated successfully into ~/.hermes/config.yaml.");
            }
            Err(e) => {
