use hermes_core::config::{load_config, save_config, has_any_provider_configured, get_env_value};
...
pub fn run_setup() {
    ...
    let hermes_home = hermes_core::paths::display_hermes_home();
...
fn setup_model_provider(config: &mut hermes_core::config::HermesConfig) {
    ...
    if !env_key.is_empty() {
        ...
        let url = url.trim();
        if !url.is_empty() {
            let _ = hermes_core::config::save_env_value("OPENAI_BASE_URL", url);
...
fn setup_terminal_backend(config: &mut hermes_core::config::HermesConfig) {
...
fn setup_agent_settings(config: &mut hermes_core::config::HermesConfig) {
...
fn setup_gateway(config: &mut hermes_core::config::HermesConfig) {
...
fn print_setup_summary(config: &hermes_core::config::HermesConfig) {
    println!();
    println!("═══════════════════════════════════════════════════");
...
fn prompt_and_save_key(env_key: &str) {
    ...
    let key = key.trim();
    if !key.is_empty() {
        match hermes_core::config::save_env_value(env_key, key) {
